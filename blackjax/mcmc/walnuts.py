...
def build_kernel(
    ...
    def dense_leapfrog(state: WALNUTSState, momentum: jax.Array, step_sz: float):
        ...
        position_next = state.position + step_sz * jnp.dot(inverse_mass_matrix, momentum_half)
        logdensity_next, grad_next = logdensity_and_grad_fn(position_next)
        momentum_next = momentum_half + 0.5 * step_sz * grad_next
        return WALNUTSState(position_next, logdensity_next, grad_next), momentum_next
    ...
    def dense_micro_routine(initial_state: WALNUTSState, initial_momentum: jax.Array, macro_step_size: float):
        ...
        def body_fn(loop_state):
            ...
            def micro_step_fn(step_idx, inner_state):
                ...
                next_state, next_mom = dense_leapfrog(curr_state, curr_mom, micro_step_size)
                curr_energy = compute_dense_hamiltonian(next_state, next_mom)
                return next_state, next_mom, jnp.maximum(curr_max_h, curr_energy), jnp.minimum(curr_min_h, curr_energy)
                
            final_state, final_mom, max_h, min_h = jax.lax.fori_loop(0, l, micro_step_fn, (initial_state, initial_momentum, initial_energy, initial_energy))
            return i + 1, max_h - min_h, l, final_state, final_mom
            
        init_loop_state = (0, jnp.inf, 1, initial_state, initial_momentum)
        _, final_error, optimal_l, final_state, final_mom = jax.lax.while_loop(condition_fn, body_fn, init_loop_state)
        return optimal_l, final_state, final_mom, final_error

    def extend_dense_trajectory(rng_key, init_state: WALNUTSState, init_mom: jax.Array, direction, num_macro_steps, init_energy):
        def loop_condition(loop_state):
            return (loop_state[0] < num_macro_steps) & jnp.logical_not(loop_state[7])
            
        def step_body(loop_state):
            step_idx, curr_state, curr_mom, prop_state, prop_mom, curr_log_w, mom_sum, is_div, is_turn, unhalved_c, key = loop_state
            step_l, next_state, next_mom, actual_error = dense_micro_routine(curr_state, curr_mom, step_size * direction)
            ...
            return (step_idx + 1, next_state, next_mom, new_prop_state, new_prop_mom, new_log_w, new_mom_sum, new_is_div, new_is_turn, unhalved_c + was_unhalved, key)
            
        init_loop = (0, init_state, init_mom, init_state, init_mom, -jnp.inf, init_mom, False, False, 0.0, rng_key)
        final_loop = jax.lax.while_loop(loop_condition, step_body, init_loop)
        _, final_bound_s, final_bound_m, final_prop_s, final_prop_m, tot_log_w, tot_mom_sum, div, turned, fin_unhalved, _ = final_loop
        return final_bound_s, final_bound_m, final_prop_s, final_prop_m, tot_log_w, tot_mom_sum, div, fin_unhalved, turned
    ...
    def step(rng_key: jax.Array, curr_state: WALNUTSState) -> Tuple[WALNUTSState, WALNUTSInfo]:
        key_mom, key_tree = jax.random.split(rng_key)
        init_mom = sample_dense_momentum(key_mom)
        init_energy = compute_dense_hamiltonian(curr_state, init_mom)
        
        def tree_condition(loop_state):
            return (loop_state[0] < max_tree_depth) & jnp.logical_not(loop_state[9]) & jnp.logical_not(loop_state[10])
            
        def tree_body(loop_state):
            depth, left_s, left_m, right_s, right_m, prop_s, prop_m, log_tot_w, mom_sum, is_turn, is_div, unh_c, key = loop_state
            key, dir_key, acc_key = jax.random.split(key, 3)
            direction = jnp.where(jax.random.uniform(dir_key) < 0.5, -1, 1)
            L = 2 ** depth
            
            def build_left(_): return extend_dense_trajectory(acc_key, left_s, left_m, -1, L, init_energy)
            def build_right(_): return extend_dense_trajectory(acc_key, right_s, right_m, 1, L, init_energy)
            
            bound_s, bound_m, new_prop_s, new_prop_m, log_sub_w, sub_mom, div, sub_unh, sub_turn = jax.lax.cond(direction == -1, build_left, build_right, operand=None)
            ...
            turning = check_dense_u_turn(new_left_m, new_right_m, new_mom_sum) | sub_turn
            return (depth + 1, new_left_s, new_left_m, new_right_s, new_right_m, final_prop_s, final_prop_m, new_log_tot_w, new_mom_sum, turning, is_div | div, unh_c + sub_unh, key)
        
        init_loop = (0, curr_state, init_mom, curr_state, init_mom, curr_state, init_mom, 0.0, init_mom, False, False, 0.0, key_tree)
        fin_depth, _, _, _, _, fin_state, _, _, _, is_turn, is_div, fin_unh, _ = jax.lax.while_loop(tree_condition, tree_body, init_loop)
        
        out_state = tree_util.tree_map(lambda old, new: jnp.where(is_div, old, new), curr_state, fin_state)
